...
int calc_cycles_increase(const int cycles, const int cycle_up)
{
	auto new_cycles = [&] {
		if (cycle_up < 100) {
...
void sync_modern_cycles_settings()
{
	auto& conf = modern_cycles_config;
...
int calc_cycles_decrease(const int cycles, const int cycle_down)
{
	auto new_cycles = [&] {
		if (cycle_down < 100) {
	...
	if (legacy_cycles_mode) {
		...
		if (cpu.pmode) {
			if (conf.protected_mode_auto) {
				...
				// mode
				s += format_str("%d", *conf.real_mode);
			} else {
				if (conf.protected_mode) {
					s += format_str("%d", *conf.protected_mode);
				} else {
					s += "max";
					max_mode = true;
				}
